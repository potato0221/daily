...
class Rq {
	...
	constructor() {
		...
		let exercise: boolean = $state(false);
		let doStudy: boolean = $state(false);
		let dailyAl: number = $state(0);
		...
		this.member = {
			...
			set doStudy(value: boolean) {
				doStudy = value;
			},
			get dailyAl() {
				return dailyAl;
			},
			set dailyAl(value: number) {
				dailyAl = value;
			}
		};
	}
	...
	public setLogined(member: components['schemas']['MemberDto']) {
		...
		this.member.exercise = member.exercise;
		this.member.doStudy = member.doStudy;
		this.member.dailyAl = member.dailyAl;
	}
